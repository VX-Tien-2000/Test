...
if input_type.lower() in ('tính 2 biến','tinh 2 bien','2','2 bien','2 biến','2bien','2b','hai bien','haibien','hai biến'):
# Loop
    while True:
        input1 = input('Nhập phép tính cần tính (Cộng/Trừ/Nhân/Chia): ')

        if input1.lower() in ('cộng','trừ','nhân','chia','cong','tru','nhan','c','tr','nh','ch','+','-','x','*','/'):
        ...
        if next_one.lower() in ('không','khong','kh','k','ko',):
            break
elif input_type.lower() in ('tính 3 biến','tinh 3 bien','3','3 bien','3 biến','3bien','3b','ba bien','babien','ba biến'):
# Loop phép tính nhiều 3 biến
    while True:
        ...
        if next_one.lower() in ('không', 'khong', 'kh', 'k', 'ko',):
            break
elif input_type.lower() in ('tính %','tinh %','tính phần trăm','tinh phan tram','%','tinh%','phan tram','phần trăm','tinhphantram','phantram'):
# Loop tính %
    while True:
